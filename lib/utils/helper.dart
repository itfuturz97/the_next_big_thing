...
class Helper {
  ...
  void shareOnWhatsApp(String message, String phoneNumber) async {
    try {
      ...
      if (await canLaunchUrl(whatsappUri)) {
        ...
        toaster.error("WhatsApp is not installed on this device");
      }
    } catch (e) {
      toaster.error("Failed to open WhatsApp: ${e.toString()}");
    }
  }
      ...
      child: Column(
        ...
        children: [
          SizedBox(
            height: size ?? 25,
            width: size ?? 25,
            child: CircularProgressIndicator(strokeWidth: strokeWidth ?? 3, valueColor: AlwaysStoppedAnimation<Color>(color ?? Colors.black), color: color ?? Colors.black),
          ),
        ],
      ),
    );
  ...
  String scheduleTimeFormat(String time) {
    if (time.isNotEmpty) {
      try {
